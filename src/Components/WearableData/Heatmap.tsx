...
interface ImagePlotCanvasProps {
  ...
  onAnimationEnd?: () => void; // Nueva prop para notificar al padre
}
  ...
  (
    {
      ...
      onAnimationEnd, // Nueva prop
    }: ImagePlotCanvasProps,
    ref,
  ) => {
    ...
    const colorStops = useMemo(() => colors as ColorStop[], []);
    ...
    // Función de animación con cálculo de FPS
    const animate = useCallback(
      (timestamp: number) => {
        ...
        const fpsElapsed = timestamp - fpsLastFrameTimeRef.current;
        if (fpsElapsed >= 1000) {
          ...
          fpsLastFrameTimeRef.current = timestamp;
        }
        ...
        while (updateAccumulator.current >= updateInterval) {
          if (
            points[0] &&
            currentFrameRef.current < points[0].values.length - 1
          ) {
            ...
            if (onAnimationEnd) {
              onAnimationEnd(); // Notificar al padre que la animación ha terminado
            }
            break;
          }
